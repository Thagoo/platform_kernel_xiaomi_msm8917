...
static ssize_t gt9xx_ts_disable_keys_show(struct device *dev,
...
static ssize_t gt9xx_ts_disable_keys_store(struct device *dev,
...
static DEVICE_ATTR(disable_keys, S_IWUSR | S_IRUSR, gt9xx_ts_disable_keys_show,
		   gt9xx_ts_disable_keys_store);

static struct attribute *gt9xx_ts_attrs[] = {
    &dev_attr_disable_keys.attr,
	NULL
};

static const struct attribute_group gt9xx_ts_attr_group = {
       .attrs = gt9xx_ts_attrs,
};

static int gt9xx_ts_proc_init(struct kernfs_node *sysfs_node_parent)
{
       int ret = 0;
       char *buf, *path = NULL;
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_group(&client->dev.kobj, &gt9xx_ts_attr_group);
        if (ret) {
	        dev_err(&client->dev, "Failure %d creating sysfs group\n",
		        ret);
                sysfs_remove_group(&client->dev.kobj, &gt9xx_ts_attr_group);
        }
	gt9xx_ts_proc_init(client->dev.kobj.sd);
